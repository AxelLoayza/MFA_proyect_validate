...
async function sendInviteEmail(to, tenantKey, tenant, name) {
...
router.post('/', async (req, res) => {
  try {
    ...
    const tenantKeyValue = tenant.tenantKey || tenantKey
    const code = generateInviteCode(tenantKeyValue)
    ...
    const invite = await TenantInvite.create({ tenantId: tenant._id, inviteCode: hashed, email, name: name || null, role: inviteRole, status: 'pending', createdAt: now, expiresAt })

    // attempt to send email (do not fail overall if mail fails?)
    try {
      await sendInviteEmail(email, tenantKeyValue, tenant, name)
    } catch (mailErr) {
