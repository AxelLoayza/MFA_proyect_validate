...
const INVITE_ROLES = new Set(['user', 'admin']);

function normalizeInviteRole(role) {
  const normalized = String(role || 'user').toLowerCase().trim();
  return INVITE_ROLES.has(normalized) ? normalized : 'user';
}
const fs = require('fs');
...
const PRIVATE_LSTM_URL = requiredEnv('PRIVATE_LSTM_URL');
const PRIVATE_LSTM_USERNAME = requiredEnv('ML_SERVICE_USERNAME');
const PRIVATE_LSTM_PASSWORD = requiredEnv('ML_SERVICE_PASSWORD');
const GOOGLE_REDIRECT_URI = requiredEnv('GOOGLE_REDIRECT_URI');
...
async function fetchMasterFeature(signatures) {
  const response = await fetch(`${PRIVATE_LSTM_URL}/api/biometric/enroll`, {
    method: 'POST',
    headers: {
  ...
  if (!response.ok) {
    ...
    error.code = payload.error || payload.code || 'enrollment_normalization_failed';
...
function getServiceApiKeyFromRequest(req) {
  return (req.headers['x-api-key'] || req.headers['x-sdk-key'] || '').toString().trim();
}

function assertServiceApiKey(req) {
  const expectedApiKey = (process.env.SERVICE_API_KEY || '').trim();

  if (!expectedApiKey) {
    return;
  }

  const providedApiKey = getServiceApiKeyFromRequest(req);

  if (!providedApiKey || providedApiKey !== expectedApiKey) {
    const error = new Error('service_api_key_invalid');
...
function getSigningPrivateKey() {
  ...
  if (!privateKeyPath) {
    const error = new Error('JWT_PRIVATE_KEY_PATH is required');
...
function getVerificationPublicKey() {
  const publicKeyPath = process.env.JWT_PUBLIC_KEY_PATH;
  if (!publicKeyPath) {
    const error = new Error('JWT_PUBLIC_KEY_PATH is required');
    error.statusCode = 500;
    throw error;
  }
...
function signServerToken(payload, expiresInSeconds) {
...
function verifyServerToken(token) {
  const publicKey = getVerificationPublicKey();
...
function prepareStepUpSignature(body) {
  const strokePoints = body?.stroke_points || body?.strokePoints || null;

  if (!Array.isArray(strokePoints)) {
    const error = new Error('stroke_points required');
    error.statusCode = 400;
    throw error;
  }

  if (strokePoints.length < 100) {
    const error = new Error('stroke_points must contain at least 100 points');
    error.statusCode = 400;
    throw error;
  }
  ...
  return {
    ...
    stroke_duration_ms: Number(body?.stroke_duration_ms || body?.strokeDurationMs || 0),
    real_length: Number(body?.real_length || body?.realLength || strokePoints.length),
  };
}

async function callPrivateLstmValidation(stepUpSignature) {
  const response = await fetch(`${PRIVATE_LSTM_URL}/api/biometric/validate`, {
    ...
    body: JSON.stringify(stepUpSignature)
  });
  ...
  if (!response.ok) {
    ...
    error.statusCode = response.status;
    error.code = payload.error || payload.code || (response.status === 401 ? 'private_auth_failed' : 'private_validation_failed');
    error.details = payload;
    throw error;
  }
...
router.post('/api/token/sign', async (req, res) => {
  try {
    assertServiceApiKey(req);

    const payload = req.body?.payload || {};
    const expiresInSeconds = parseInt(req.body?.expiry_seconds || req.body?.expires_in || '300', 10);

    if (!Number.isFinite(expiresInSeconds) || expiresInSeconds <= 0) {
      return res.status(400).json({ error: 'invalid_expiry_seconds', message: 'expiry_seconds must be a positive integer' });
    }

    const token = signServerToken(payload, expiresInSeconds);
    ...
    return res.status(statusCode).json({
      error: err.message || 'token_sign_failed'
    });
  }
});
...
router.post('/api/token/verify', async (req, res) => {
  try {
    ...
    const statusCode = err.name === 'TokenExpiredError' ? 401 : (err.statusCode || 401);
    return res.status(statusCode).json({
      valid: false,
      error: err.message || 'token_verify_failed'
    });
  }
});
...
router.post('/google', async (req, res) => {
  try {
    ...
    if (user && action === 'register') {
      const requestedTenantKey = String(req.body.tenantKey || '').trim();
      const currentTenant = getTenantDetails(user);
      const currentTenantKey = currentTenant?.tenantKey || user.tenantKey || null;

      if (!requestedTenantKey || !currentTenantKey || requestedTenantKey !== currentTenantKey) {
        return res.status(409).json({ error: 'already_registered', message: 'Este correo ya está registrado. Usa iniciar sesión.' });
      }

      console.log(`[Cloud Service] Usuario ya registrado, tenantKey coincide (${requestedTenantKey}). Continuando con sesión existente.`);
    }
...
router.post('/google/verify-access', async (req, res) => {
  try {
    ...
    if (!user) {
      ...
      if (invite) {
        const tenant = await Tenant.findById(invite.tenantId).lean();
        const inviteRole = normalizeInviteRole(invite.role);
        const tenantContextDocument = buildTenantContextDocument(tenant || { _id: invite.tenantId, tenantKey: null }, { role: inviteRole, status: 'active' });
        user = await User.create({
          googleId,
          email,
          name: name || invite.name || '',
...
router.post('/enroll', async (req, res) => {
  try {
    ...
    const biometricPayload = req.body?.master_feature || req.body?.biometric_template || null;
    const signatures = Array.isArray(req.body?.signatures) ? req.body.signatures : null;
    const samplesUsed = signatures ? signatures.length : (req.body?.samplesUsed || 5);

    if (!token) {
      return res.status(401).json({ error: 'Token requerido' });
    }

    if (!biometricPayload && !signatures) {
      return res.status(400).json({
    ...
    const resolvedBiometricPayload = biometricPayload || (signatures ? await fetchMasterFeature(signatures) : null);
    const masterFeature = resolvedBiometricPayload?.master_feature || resolvedBiometricPayload;

    const enrollmentTemplate = {
      templateVersion: 'arc_signature_template_v1',
      preprocessingProfile: 'repo_compat',
      representationStrategy,
      templateShape: masterFeature?.dtw_medoid ? 'dtw_medoid' : 'unknown',
      samplesUsed,
      masterFeature
    };
...
router.post('/step-up', async (req, res) => {
  try {
    const token = getBearerToken(req);
    if (!token) {
      return res.status(401).json({ error: 'token_required', message: 'Token requerido' });
    }

    const stepUpSignature = prepareStepUpSignature(req.body);
    ...
    try {
      ...
      return res.status(401).json({
        error: 'invalid_token',
        message: 'Token ARC 0.5 inválido o expirado',
        details: { reason: err.message }
      });
    }

    console.info('[Cloud Service] Step-up token claims:', {
      sub: decoded?.sub || null,
      arc: resolveArcLevel(decoded),
    ...
    const normalizedReferenceTemplate = referenceTemplate?.masterFeature || referenceTemplate?.master_feature || referenceTemplate;
    stepUpSignature.reference_template = normalizedReferenceTemplate;

    const privateResult = await callPrivateLstmValidation(stepUpSignature);
    ...
    if (err.statusCode) {
      const responseError = err.code || (err.statusCode === 401 ? 'invalid_token' : err.statusCode === 403 ? 'insufficient_arc' : 'step_up_failed');
      return res.status(err.statusCode).json({
        error: responseError,
...
router.get('/me', authenticate, async (req, res) => {
  try {
    ...
    res.json({
      arc: resolveArcLevel(req.serverJwt),
      user: buildArcUserResponse(user)
    });
  } catch (err) {
    console.error(err);
