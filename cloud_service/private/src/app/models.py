...
class StepUpBiometricRequest(BaseModel):
    """Raw step-up payload forwarded from the public gateway."""
    timestamp: str = Field(..., description="ISO 8601 timestamp of capture")
    stroke_points: List[StrokePoint] = Field(
        ...,
        min_length=100,
        max_length=1200,
        description="Raw signature stroke points"
    )
    stroke_duration_ms: int = Field(..., description="Total duration in milliseconds", ge=0)
    real_length: int = Field(..., description="Original length before any padding", ge=100)
    reference_template: Optional[Dict[str, Any]] = Field(
        None,
        description="Enrollment template used for step-up comparison"
    )
...
class EnrollmentSignatureRequest(BaseModel):
    """Raw signature payload used to compute the master feature."""
    timestamp: str = Field(..., description="ISO 8601 timestamp of capture")
    stroke_points: List[StrokePoint] = Field(
        ...,
        min_length=100,
...
class EnrollmentCloudRequest(BaseModel):
    """Request para generar el Feature Maestro con 5 firmas crudas"""
    signatures: List[EnrollmentSignatureRequest] = Field(
        ...,
        min_length=5,
        max_length=5,
        description="5 firmas crudas para extraer master feature"
    )
...
class MasterFeatureResponse(BaseModel):
    ...
    master_feature: Dict[str, Any] = Field(
        ...,
        description="Contiene el template maestro y metadatos asociados"
    )
