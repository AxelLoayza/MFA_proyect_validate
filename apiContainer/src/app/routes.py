...
async def enroll_biometric_master(request: Request, payload: EnrollmentRequest) -> EnrollmentResponse:
    """
    ...
    """
    logger.info(f"Recibiendo solicitud de enrolamiento con {len(payload.signatures)} firmas")
    ...
    try:
        ...
        cloud_response = send_enrollment_to_ml_service(
            raw_signatures,
            payload.representation_strategy,
...
@router.post("/api/auth/step-up", dependencies=[Depends(rate_limit_dependency)])
async def step_up_signature_login(
    ...
    try:
        raw_signature = {
            ...
            "stroke_points": [{"x": p.x, "y": p.y, "t": p.t, "p": p.p} for p in request.stroke_points],
            "stroke_duration_ms": request.stroke_duration_ms,
            "real_length": len(request.stroke_points),
        }

        return forward_step_up_to_public_gateway(
            signature_payload=raw_signature,
...
async def exchange_google_code(request: dict) -> dict:
    ...
    {
      "code": "4/0AY0e-g...",
            "redirect_uri": "<GOOGLE_REDIRECT_URI from env>"
    }
