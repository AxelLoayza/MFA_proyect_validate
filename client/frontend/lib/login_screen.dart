...
class _LoginScreenState extends State<LoginScreen> {
  ...
  final String _publicBackendUrl = resolvePublicBackendUrl();
  ...
  Future<void> _signInWithGoogle() async {
    ...
    try {
      ...
      setState(() {
        _statusMessage = rawMessage;
        _isLoading = false;
      });
      _showMessage('Error en inicio de sesión: $rawMessage');

      if (kDebugMode) {
        debugPrint('[LoginScreen] Error: $rawMessage');
      }
    }
  }
  ...
  bool _isBiometricEnrolled(Map<String, dynamic> user) {
    if (user['biometricEnrolled'] == true) {
      return true;
    }

    if (user['biometricProfileId'] != null) {
      return true;
    }

    final biometricTemplate = user['biometricTemplate'];
    return biometricTemplate is Map && biometricTemplate.isNotEmpty;
  }
  ...
  Future<void> _continueToNextStepAfterLogin() async {
    ...
    if (user == null) {
      final hasBiometric = _biometricEnrolled;
    ...
    final hasBiometric = _biometricEnrolled || _isBiometricEnrolled(user);
